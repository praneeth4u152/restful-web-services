/*
 * package com.myproj.rest.webservices.restfulwebservices.jwt.resource;
 * 
 ...
 * import
 * com.in28minutes.rest.webservices.restfulwebservices.jwt.JwtUserDetails;
 * 
 * @RestController
 * 
 ...
 * 
 * @Value("${jwt.http.request.header}") private String tokenHeader;
 * 
 ...
 * JwtTokenRequest authenticationRequest) throws AuthenticationException {
 * 
 ...
 * 
 * @RequestMapping(value = "${jwt.refresh.token.uri}", method =
 * RequestMethod.GET) public ResponseEntity<?>
 * refreshAndGetAuthenticationToken(HttpServletRequest request) { String
 * authToken = request.getHeader(tokenHeader); final String token =
 * authToken.substring(7); String username =
 * jwtTokenUtil.getUsernameFromToken(token); JwtUserDetails user =
 * (JwtUserDetails) jwtInMemoryUserDetailsService.loadUserByUsername(username);
 * 
 * if (jwtTokenUtil.canTokenBeRefreshed(token)) { String refreshedToken =
 * jwtTokenUtil.refreshToken(token); return ResponseEntity.ok(new
 * JwtTokenResponse(refreshedToken)); } else { return
 ...
 * (DisabledException e) { throw new AuthenticationException("USER_DISABLED",
 * e); } catch (BadCredentialsException e) { throw new
