/*
 * package com.myproj.rest.webservices.restfulwebservices.jwt;
 * 
 ...
 * 
 * import io.jsonwebtoken.Claims; import io.jsonwebtoken.Clock; import
 * io.jsonwebtoken.Jwts; import io.jsonwebtoken.SignatureAlgorithm; import
 * io.jsonwebtoken.impl.DefaultClock;
 * 
 * @Component public class JwtTokenUtil implements Serializable {
 * 
 * static final String CLAIM_KEY_USERNAME = "sub"; static final String
 * CLAIM_KEY_CREATED = "iat"; private static final long serialVersionUID =
 * -3301605591108950415L; private Clock clock = DefaultClock.INSTANCE;
 * 
 * @Value("${jwt.signing.key.secret}") private String secret;
 * 
 * @Value("${jwt.token.expiration.in.seconds}") private Long expiration;
 * 
 ...
 * getExpirationDateFromToken(token); return expiration.before(clock.now()); }
 * 
 * private Boolean ignoreTokenExpiration(String token) { // here you specify
 * tokens, for that the expiration is ignored return false; }
 * 
 ...
 * final Date createdDate = clock.now(); final Date expirationDate =
 ...
 * 
 * public String refreshToken(String token) { final Date createdDate =
 * clock.now(); final Date expirationDate =
 * calculateExpirationDate(createdDate);
 * 
 * final Claims claims = getAllClaimsFromToken(token);
 * claims.setIssuedAt(createdDate); claims.setExpiration(expirationDate);
 * 
 * return Jwts.builder().setClaims(claims).signWith(SignatureAlgorithm.HS512,
 * secret).compact(); }
 * 
 * public Boolean validateToken(String token, UserDetails userDetails) {
 * JwtUserDetails user = (JwtUserDetails) userDetails; final String username =
 * getUsernameFromToken(token); return (username.equals(user.getUsername()) &&
 * !isTokenExpired(token)); }
 * 
 * private Date calculateExpirationDate(Date createdDate) { return new
 * Date(createdDate.getTime() + expiration * 1000); } }
 */